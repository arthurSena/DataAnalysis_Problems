    ...
    #Criando a coluna distância
     ...
     for (i in 1:nrow(dados)){
         if (i == 1){
           ...
           if (dados[i,1] == temp_veic){
              #Usando a distância euclidiana abaixo
                dados[i,6] = dados[i-1,6] + sqrt((dados[i,2] - dados[i-1,2])^2 + (dados[i,3] - dados[i-1,3])^2)
           }
           else{
              dados[i,6] = 0
              temp_veic <- dados[i,1]
...
```{r}
    #Criando o timestamp a partir da variável DTHR
     dados$timestamp <- as.POSIXct(strptime(x = as.character(dados$DTHR),
                                 format = "%d/%m/%Y %H:%M:%S"))
     dados$Duracao <- NA
     temp_veic <- ""
         ...
         if (i == 1){
           ...
           if (dados[i,1] == temp_veic){
                    #Subtraímos o tempo atual pelo anterior e somamos com o tempo até o ponto inicial
                      dados[i,8] = dados[i-1,8] + (dados[i-1,7]-dados[i,7])
           }
           else{
              dados[i,8] = 0
              temp_veic <- dados[i,1]
...
```{r, message=FALSE, warning=FALSE}
    hist(filter(dados, VEIC == "AA005")$Duracao)
    hist(filter(dados, VEIC == "AA006")$Duracao)
    hist(filter(dados, VEIC == "AA007")$Duracao)
```
